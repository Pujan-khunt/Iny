...
export interface RetrievalDeps {
  db: { execute: <T extends Record<string, unknown>>(query: any) => Promise<{ rows: T[] }> };
...
export function createRetriever(deps: RetrievalDeps) {
  return {
    async retrieveTopK(
      ...
      const vectors = await deps.embedClient.embedBatch([trimmedQuery]);
      ...
      const result = await deps.db.execute<HybridRow>(querySql);
...
let defaultRetrieverInstance: ReturnType<typeof createRetriever> | null = null;
export const retrieveTopK = async (query: string, opts?: { threshold?: number; topK?: number }) => {
  if (!defaultRetrieverInstance) {
    defaultRetrieverInstance = createRetriever({ db: db as any, embedClient: new OpenAIEmbeddingClient() });
  }
  return defaultRetrieverInstance.retrieveTopK(query, opts);
};